...
HeapTop=$(grep "__HeapTop" "$1" | grep -v ASSERT | sed -n 's/.*= *\(0x[0-9A-Fa-f]*\).*/\1/p')
HeapBase=$(grep "__HeapStart" "$1" | grep -v ASSERT | sed -n 's/.*= *\(0x[0-9A-Fa-f]*\).*/\1/p')

# Convert hex to decimal for safe arithmetic
HeapTopDec=$((16#${HeapTop#0x}))   # remove 0x if present
HeapBaseDec=$((16#${HeapBase#0x}))

# Compute maximum heap size
printf "HEAP:\t\t0x%X\t0x%X\t0x%X\n" $HeapTopDec $((HeapBaseDec)) $((HeapTopDec - HeapBaseDec))
printf "\nMaximum heap size: 0x%X\n\n" $((HeapTopDec - HeapBaseDec + 1))
