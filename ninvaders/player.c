...
void playerLaunchMissile()
{
	...
	if (player.missileFired == 0) {
		xEventGroupSetBits(ninvaders_to_effect_events, SHOOT_EVENT);
...
int playerMoveMissile()
{
	...
	if (player.missileFired == 1) {
		...
		if (alienTypeHit != 0) {
			xEventGroupSetBits(ninvaders_to_effect_events, INVADERKILLED_EVENT);
 ...
 */
void playerExplode(){
	xEventGroupSetBits(ninvaders_to_effect_events, EXPLOSION1_EVENT);
