...
void setUpTimer()
{
	...
	timer = xTimerCreateStatic("ninvaders timer", //name for humans
							pdMS_TO_TICKS(1000 / FPS ), //period to trigger
							pdTRUE, //auto reload to make looping
							(void *) 0, //don't need identifier
							handleTimer,
							&timer_buffer);

	xTimerStart( timer, portMAX_DELAY); //start timer. wait a long time
	//old timer code
...
void ninvaders_task(void *pvParameters)
{
	...
	//TickType_t experation = xTimerGetExpiryTime( timer );
    ...
    while (1); //just in case it get here
}
